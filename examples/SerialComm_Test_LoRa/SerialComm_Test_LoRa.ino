/*  SerialComm.ino
 *  Author: Alex St. Clair / Lars Kalnajs
 *  Created: July 20202
 *  A test of using a LoRa Stream object with SerComm
 */
...
uint8_t bin_tx[] = "Readable test binary";
...
void setup()
{
  ...
  //Setup LoRa module (HELTEC ESP32)
  SPI.begin(SCK,MISO,MOSI,SS);
  LoRa.setPins(SS,RST_LoRa,30);
  if (!LoRa.begin(BAND))
    Serial.print("Starting LoRa failed!\r\n");  
  
  LoRa.setSignalBandwidth(250E3); //250KHz BW
  Serial.println("Bandwidth = 250");
  LoRa.setSpreadingFactor(11); // SF = 11
  Serial.println("Spreading Factor = 11");
  LoRa.setTxPower(10); //power = 14dbm (~10mW)
  delay(1000);
...
void loop()
{
   ...
   ser.TX_ASCII(32);
//  LoRa.beginPacket();
//  //int sent = LoRa.write(bin_tx,bin_tx_length);
//  int sent = LoRa.print("#32;50116;");
//  LoRa.endPacket(true);
//  Serial.print("Bytes sent: "); Serial.println(sent);
//  if( sent)
//  {
